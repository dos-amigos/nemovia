...
function extractDetailContent($: cheerio.CheerioAPI, sourceName: string): DetailContent {
  switch (sourceName) {
    ...
    case "sagretoday": return extractSagretodayDetail($);
...
// --- Section 4b: Custom API-based scrapers (TIER 1+2) ---
// These sources use JSON APIs or non-standard HTML, so they bypass the generic
// Cheerio-based scrapeSource() path. They reuse normalizeRawEvent(), upsertEvent(),
// and all filter functions.

const VENETO_PROVINCE_CODES = ["BL", "PD", "RO", "TV", "VE", "VI", "VR"];

// Map full province names to codes (sagriamo API sometimes returns full names)
...
// --- trovasagre.it ---
// JSON API at backend-agg.php?action=sagre returns ALL sagre; filter client-side by regione.
// Fields: nome_sagra, descrizione, comune_nome, provincia, regione, data_inizio, data_fine,
//         web, foto[], indirizzo, frazione
async function scrapeTrovasagre(supabase: SupabaseClient): Promise<void> {
  ...
  // Create a synthetic source object for logging
  const syntheticSource: ScraperSource = {
    id: "custom-trovasagre",
    name: sourceName,
    display_name: "TrovaSagre",
    base_url: "https://www.trovasagre.it",
    selector_item: "", selector_title: "",
    selector_start_date: null, selector_end_date: null,
    selector_city: null, selector_price: null,
    selector_url: null, selector_image: null,
    url_pattern: null, next_page_selector: null,
    max_pages: 1, is_active: true, consecutive_failures: 0,
  };
  ...
  try {
    ...
    for (const item of venetoItems) {
      ...
      // Build city string: "ComuneNome (Provincia abbreviata)"
      ...
      // Image: foto is an array of relative paths like "uploads/sagra_xxx.jpg"
      ...
      // Source URL: trovasagre has no detail page per se, use website if available
    ...
    await logRun(supabase, syntheticSource, "success", eventsFound, eventsInserted, eventsMerged, null, startedAt);
    console.log(`[scrapeTrovasagre] done: found=${eventsFound}, inserted=${eventsInserted}, merged=${eventsMerged}`);
  } catch (err) {
    const errorMessage = err instanceof Error ? err.message : String(err);
    console.error(`[scrapeTrovasagre] Error:`, errorMessage);
    await logRun(supabase, syntheticSource, "error", eventsFound, eventsInserted, eventsMerged, errorMessage, startedAt);
  }
}

// --- sagriamo.it ---
// REST API at https://app.sagriamo.it/api/festival/all?page={n}&perPage=100
// Returns { amount, npages, items[] }. Filter by address.province in Veneto codes.
// Rich data: name, description, address, active_from/to, logo[], cover[], gallery[],
...
async function scrapeSagriamo(supabase: SupabaseClient): Promise<void> {
  ...
  const syntheticSource: ScraperSource = {
    id: "custom-sagriamo",
    name: sourceName,
    display_name: "Sagriamo",
    base_url: "https://www.sagriamo.it",
    selector_item: "", selector_title: "",
    selector_start_date: null, selector_end_date: null,
    selector_city: null, selector_price: null,
    selector_url: null, selector_image: null,
  ...
  try {
    const PER_PAGE = 100;
    const MAX_PAGES = 10; // safety cap: 100*10 = 1000 max
    ...
    for (let page = 1; page <= MAX_PAGES; page++) {
      ...
      for (const item of items) {
        // Filter by province code for Veneto
        ...
        // Image: prefer cover[0], fallback to logo[0]
        let imageUrl: string | null = null;
        ...
        // Source URL: sagriamo detail page
        ...
        if (result === "inserted" && eventId) {
          ...
          if (desc && desc.length > 10) {
            // Strip HTML tags from description (sagriamo returns HTML in some cases)
    ...
    await logRun(supabase, syntheticSource, "success", eventsFound, eventsInserted, eventsMerged, null, startedAt);
    console.log(`[scrapeSagriamo] done: found=${eventsFound}, inserted=${eventsInserted}, merged=${eventsMerged}`);
  } catch (err) {
    const errorMessage = err instanceof Error ? err.message : String(err);
...
// --- sagretoday.it ---
// Next.js site with JSON-LD structured data (schema.org/ItemList).
// Strategy: scrape /sagre/veneto/{province}/page/{n}/ for each province.
// Extract events from JSON-LD <script type="application/ld+json"> containing ItemList.
// Fallback: scrape detail pages at /sagra/{slug}--e_{id}/ for richer data (JSON-LD Event).
const SAGRETODAY_PROVINCES = ["belluno", "padova", "rovigo", "treviso", "venezia", "vicenza", "verona"];

async function scrapeSagretoday(supabase: SupabaseClient): Promise<void> {
  ...
  const syntheticSource: ScraperSource = {
    id: "custom-sagretoday",
    name: sourceName,
    display_name: "SagreToday",
    base_url: "https://www.sagretoday.it",
    selector_item: "", selector_title: "",
    selector_start_date: null, selector_end_date: null,
    selector_city: null, selector_price: null,
    selector_url: null, selector_image: null,
  ...
  try {
    for (const province of SAGRETODAY_PROVINCES) {
      const MAX_PAGES = 5; // safety cap per province
      ...
      for (let page = 1; page <= MAX_PAGES; page++) {
        ...
        jsonLdScripts.each((_i: number, el: cheerio.Element) => {
          try {
            ...
            // Look for ItemList with Event items
            ...
            for (const eventItem of elements) {
              // eventItem might be an Event directly or wrapped in ListItem
              ...
              // Location: from event.location.name or event.location.address.addressLocality
              let city = "";
              if (event.location) {
              ...
              // Dates: ISO format from startDate/endDate
              let startDate: string | null = null;
              let endDate: string | null = null;
              if (event.startDate) {
                startDate = String(event.startDate).slice(0, 10); // YYYY-MM-DD
              }
              if (event.endDate) {
              ...
              // Source URL: from event.url or construct from event data
        ...
        // Process collected JSON-LD events sequentially (async-safe)
        ...
        // SagreToday uses <a> tags with href="/sagra/{slug}--e_{id}/" pattern
        if (!itemListFound) {
          ...
          links.each((_i: number, el: cheerio.Element) => {
            ...
            // Match pattern: /sagra/{slug}--e_{id}/
            ...
            // City from the province context
            const city = province;
            ...
            fallbackEvents.push({
              ...
              imageUrl: null,
              url: sourceUrl,
              contentHash: generateContentHash(title, city, null),
            });
          });

          // Process fallback events sequentially
    ...
    // Detail page scraping for sagretoday
    const detailUrls = await getEventsNeedingDetails(supabase, sourceName, 10);
    if (detailUrls.length > 0) {
      const { scraped, updated } = await scrapeDetailPages(supabase, sourceName, detailUrls);
      console.log(`[scrapeSagretoday] detail pages scraped=${scraped}, updated=${updated}`);
    }

    await logRun(supabase, syntheticSource, "success", eventsFound, eventsInserted, eventsMerged, null, startedAt);
    console.log(`[scrapeSagretoday] done: found=${eventsFound}, inserted=${eventsInserted}, merged=${eventsMerged}`);
  } catch (err) {
    const errorMessage = err instanceof Error ? err.message : String(err);
    console.error(`[scrapeSagretoday] Error:`, errorMessage);
    await logRun(supabase, syntheticSource, "error", eventsFound, eventsInserted, eventsMerged, errorMessage, startedAt);
  }
}

// --- Detail extractors for new sources ---

function extractSagretodayDetail($: cheerio.CheerioAPI): DetailContent {
  // SagreToday detail pages have JSON-LD Event schema with description
  let description: string | null = null;
  let orari: string | null = null;
...
async function runPipeline(supabase: SupabaseClient) {
  // Phase 1: Run existing DB-driven scrapers
  ...
  if (error || !sources?.length) {
    ...
    // Don't return — still run custom scrapers below
  } else {
  ...
  // Phase 2: Run custom API-based scrapers (TIER 1+2)
  console.log("[runPipeline] Starting custom scrapers (trovasagre, sagriamo, sagretoday)...");

  // Run all three custom scrapers sequentially (to be polite with resources)
  await scrapeTrovasagre(supabase);
  await scrapeSagriamo(supabase);
  await scrapeSagretoday(supabase);

  console.log("[runPipeline] All scrapers complete.");
}
